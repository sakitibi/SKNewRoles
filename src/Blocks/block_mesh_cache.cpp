...
Ref<Material> BlockMeshCache::default_material;
...
Ref<Material> BlockMeshCache::get_default_material() {
    ...
    mat.instantiate();
    mat->set_albedo(Color(1.0f, 0.0f, 1.0f));
    default_material = mat;
    return default_material;
}

BlockMeshData BlockMeshCache::load_single_block_mesh(const String &scene_path) {
    BlockMeshData data;
    if (scene_path.is_empty() || !ResourceLoader::get_singleton()->exists(scene_path)) {
        return data;
    }

    Ref<PackedScene> scene = ResourceLoader::get_singleton()->load(scene_path);
    if (scene.is_null()) return data;
...
void BlockMeshCache::preload_block_meshes() {
    ...
    for (const auto &E : registry_map) {
        ...
        if (!mesh_cache.has(scene_path)) {
            BlockMeshData data = load_single_block_mesh(scene_path);
            if (data.valid) {
