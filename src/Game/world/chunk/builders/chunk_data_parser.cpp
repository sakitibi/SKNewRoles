            ...
            for (int z = 0; z < 16; ++z) {
                for (int x = 0; x < 16; ++x) {
                    ...
                    if (!data_array.is_empty()) {
                        p_idx = get_palette_index(data_array, palette_size, x, y, z);
                    }
                    ...
                    if (p_idx >= 0 && p_idx < palette_size) {
                        ...
                        if (block_name != "minecraft:air" && BlockRegistry::has_block(block_name)) {
                            float world_y = static_cast<float>(section_y * 16 + y);
                            Vector3 pos(static_cast<float>(x), world_y, static_cast<float>(z));
