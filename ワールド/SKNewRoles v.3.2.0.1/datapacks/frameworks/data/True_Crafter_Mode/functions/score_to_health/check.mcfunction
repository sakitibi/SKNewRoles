...
execute if score #__ ScoreToHealth matches ..0 unless entity @s[tag=ScoreToHealth.AntiGlitch.UnsafeTick] run function True_Crafter_Mode:score_to_health/modify
tag @s remove ScoreToHealth.AntiGlitch.UnsafeTick
execute if score #__ ScoreToHealth matches 1.. run function True_Crafter_Mode:score_to_health/anti_glitch/protect
