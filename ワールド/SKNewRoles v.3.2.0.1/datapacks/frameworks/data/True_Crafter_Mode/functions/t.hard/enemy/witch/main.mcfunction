...
# 豚を変換
    execute as @e[type=pig,tag=!Chuz_NonVanillaMob,tag=!D.Gun_Common,distance=..8] at @s run function True_Crafter_Mode:t.hard/enemy/witch/pig_convert

# ウサギを変換
    execute as @e[type=rabbit,tag=!Chuz_NonVanillaMob,distance=..8,nbt=!{RabbitType:99}] at @s run function True_Crafter_Mode:t.hard/enemy/witch/rabbit_convert

# バフを配る
    execute as @e[type=#True_Crafter_Mode:t.hard/hostiles,type=!witch,distance=0.1..8] at @s run function True_Crafter_Mode:t.hard/enemy/witch/buff
...
# ワープ
    ...
    execute if entity @s[scores={T.HardMoveset1=40}] run function True_Crafter_Mode:t.hard/enemy/witch/teleport
