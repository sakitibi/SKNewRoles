...
execute if entity @a[distance=..7] facing entity @p eyes rotated ~ 0 unless block ^ ^-1 ^-3 #True_Crafter_Mode:t.hard/no_collision if block ^ ^ ^-1 #True_Crafter_Mode:t.hard/no_collision run function True_Crafter_Mode:t.hard/enemy/common/back_step
#一度だけしか回復しないようにタグ付与
