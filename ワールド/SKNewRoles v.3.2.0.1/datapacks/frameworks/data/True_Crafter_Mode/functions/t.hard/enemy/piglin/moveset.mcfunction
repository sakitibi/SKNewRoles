...
# 何かと敵対してると本気出す
    execute if entity @s[predicate=True_Crafter_Mode:t.hard/battle_mode] run function True_Crafter_Mode:t.hard/enemy/piglin/true_power
...
# 耐火ポーション飲み終わり
    execute if entity @s[tag=T.HardPiglin_F.Resist,scores={T.HardMoveset2=30}] run function True_Crafter_Mode:t.hard/enemy/piglin/fire_resist_finish

# 体力をスコア化
    execute store result score @s T.HardHealth run data get entity @s Health 1
# スコア化された体力が半分以下かつ、プレイヤーとの距離が開いていれば、一度のみ回復する
    execute if entity @s[scores={T.HardHealth=..10},tag=!T.HardNoHeal,tag=!T.HardPiglin_Heal] unless entity @e[type=#True_Crafter_Mode:t.hard/piglin_enemy,distance=..8,tag=!T.HardException] run function True_Crafter_Mode:t.hard/enemy/piglin/heal
...
# 回復完了
    execute if entity @s[tag=T.HardPiglin_Heal,scores={T.HardMoveset2=30}] run function True_Crafter_Mode:t.hard/enemy/piglin/heal_finish
