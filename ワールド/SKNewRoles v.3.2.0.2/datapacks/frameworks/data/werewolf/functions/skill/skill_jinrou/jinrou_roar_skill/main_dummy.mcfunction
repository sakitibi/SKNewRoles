#アイテムを飛ばす
execute anchored eyes run summon item ^ ^-0.25 ^ {Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:99999}},Health:1s,PickupDelay:10000,Tags:["roar_dummy","not_yet"],NoGravity:true,Age:5950}
...
execute as @e[type=item,tag=roar_dummy,tag=not_yet,limit=1,sort=nearest] run data modify entity @s Motion set from entity @e[type=area_effect_cloud,tag=roar_motion,limit=1] Pos
