...
#スキル発動
execute as @a[scores={charge_roar=20}] at @s run function werewolf:skill/skill_witch/witch_roar_skill/main

#チャージ用のスコアボードをリセット
execute as @a[scores={charge_roar=30..}] run scoreboard players reset @s charge_roar

#再帰
execute if entity @a[scores={charge_roar=0..}] run schedule function werewolf:skill/skill_witch/witch_roar_skill/charge 1t