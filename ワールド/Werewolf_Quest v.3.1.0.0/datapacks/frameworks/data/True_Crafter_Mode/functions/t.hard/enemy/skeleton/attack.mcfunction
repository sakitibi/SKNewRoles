...
# 背後チェックしてダッシュ/バックステップ
    ...
    execute if entity @s[tag=!T.HardSke_Ranged,scores={T.HardMoveset1=40}] facing entity @p eyes rotated ~ 0 unless block ^ ^-1 ^-3 #True_Crafter_Mode:t.hard/no_collision if block ^ ^ ^-1 #True_Crafter_Mode:t.hard/no_collision run function True_Crafter_Mode:t.hard/enemy/common/back_step
    execute if entity @s[tag=!T.HardSke_Ranged,scores={T.HardMoveset1=40}] facing entity @p eyes rotated ~ 0 unless block ^ ^ ^-1 #True_Crafter_Mode:t.hard/no_collision run function True_Crafter_Mode:t.hard/enemy/common/leap_attack
    execute if entity @s[tag=!T.HardSke_Ranged,scores={T.HardMoveset1=40}] facing entity @p eyes rotated ~ 0 if block ^ ^-1 ^-3 #True_Crafter_Mode:t.hard/no_collision run function True_Crafter_Mode:t.hard/enemy/common/leap_attack
...
# 前方にブロックがあればスコアリセット
    execute rotated ~ 0 unless block ^ ^ ^1 #True_Crafter_Mode:t.hard/no_collision run scoreboard players reset @s T.HardMoveset2
    execute rotated ~ 0 positioned ~ ~1 ~ unless block ^ ^ ^1 #True_Crafter_Mode:t.hard/no_collision run scoreboard players reset @s T.HardMoveset2
    scoreboard players set @s[scores={T.HardMoveset2=40}] Chuz_Speed 8
    execute if entity @s[scores={T.HardMoveset2=40}] rotated ~ 0 unless block ^ ^-1 ^2 #True_Crafter_Mode:t.hard/no_collision run function True_Crafter_Mode:t.hard/enemy/common/leap_attack
...
# 水の中ですいすい
    execute if entity @a[distance=..30] if entity @s[nbt={HurtTime:0s}] if block ~ ~0.5 ~ #True_Crafter_Mode:t.hard/liquid run function True_Crafter_Mode:t.hard/enemy/common/swim