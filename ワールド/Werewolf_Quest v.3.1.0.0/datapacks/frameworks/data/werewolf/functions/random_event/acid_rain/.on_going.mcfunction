...
#天候を雨に
execute unless entity @a[team=Witch] if score GameManager event_timer_countdown matches 750 run weather rain
execute if entity @a[team=Witch] if score GameManager event_timer_countdown matches 750 run weather rain

#傘を持っておらず頭上にブロックがなければウィザー状態を付与
execute if score GameManager event_timer_countdown matches ..750 run execute as @a[predicate=werewolf:have_item/umbrella] at @s positioned over motion_blocking if entity @s[dy=999] run effect give @s[team=!Witch] wither 1 0 true
